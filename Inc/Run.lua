...
basel = Token:match("(%d+)")
redis:set(basel..":VERSION",2.9)
redis:set(basel..":SUDO_ID:",SUDO_USER)
redis:set(basel..":DataCenter:",'German')
redis:set(basel..":UserNameBot:",BOT_User)
redis:set(basel..":NameBot:",BOT_NAME)
redis:hset(basel..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3basel_INSTALL","Yes")
info = {}
...
basel = Token:match("(%d+)")
our_id = tonumber(basel)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(basel..":UserNameBot:")
SUDO_ID = tonumber(redis:get(basel..":SUDO_ID:"))
SUDO_USER = redis:hgetall(basel..'username:'..SUDO_ID).username
version = redis:get(basel..":VERSION")
DataCenter = redis:get(basel..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..basel..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(basel..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(basel..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(basel..'group:add'..msg.chat_id_) then 
...
msg.Rank = 1
elseif redis:sismember(basel..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور '
msg.TheRank = 'مطور البوت '
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(basel..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي '
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(basel..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء '
msg.TheRank = 'المنشىء '
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(basel..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير ' 
msg.TheRank = 'مدير البوت ' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(basel..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن '
msg.TheRank = 'ادمن في البوت '
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(basel..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(basel..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(basel..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(basel..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(basel..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(basel..':WELCOME_BOT'),[[•~مـرحبآ آنآ بوت آسـمـي ]]..redis:get(basel..':NameBot:')..[[•~آختصـآصـي حمـآيه‌‏ آلمـجمـوعآت
...
if msg.GroupActive and redis:get(basel..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(basel..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(basel..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(basel..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.basel then 
if msg.text and ScriptFile.ibasel then
for k, basel in pairs(ScriptFile.basel) do
local SearchText = msg.text:match(basel)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.ibasel(msg,{msg.text:match(basel)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",basel.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dbasel then
if ScriptFile.dbasel(msg) == false then
return false
end
...
if Plug.basel then 
if msg.text and Plug.ibasel then
for k, basel in pairs(Plug.basel) do
local SearchText = msg.text:match(basel)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(basel..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(basel..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.ibasel(msg,{msg.text:match(basel)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",basel.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source basel")
end 

end
else
print("The File Script.lua Not Runing in The Source basel")
...
function tdcli_update_callback(data)
	...
	redis:srem(basel..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/baselTELEE/basel/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/baselTELEE/basel/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/baselTELEE/basel/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/baselTELEE/basel/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/baselTELEE/basel/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/baselTELEE/basel/master/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(basel..":SUDO_ID:")
redis:hset(basel..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(basel..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(basel..'mute_tgservice'..msg.chat_id_) then
	Del_msg(msg.chat_id_,msg.id_)
	end
	elseif msg.content_.ID == "MessageChatAddMembers" then
	if redis:get(basel..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(basel..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(basel..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(basel..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(basel..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(basel..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(basel..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(basel..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(basel..'group:add'..msg.chat_id_) then
	redis:set(basel..'group:name'..msg.chat_id_,msg.content_.title_)
	end
	end 
	
	elseif data.ID == "UpdateNewChat" then  
	if redis:get(basel..'group:add'..data.chat_.id_) then
	redis:set(basel..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(basel..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(basel..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(basel..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	basel..'group:name'..k,IDS.Title,
	basel..'num_msg_basel'..k,5,
	basel..'group:add'..k,true,
	basel..'lock_link'..k,true,
	basel..'lock_spam'..k,true,
	basel..'lock_webpage'..k,true,
	basel..'lock_markdown'..k,true,
	basel..'lock_flood'..k,true,
	basel..'lock_bots'..k,true,
	basel..'mute_forward'..k,true,
	basel..'mute_contact'..k,true,
	basel..'mute_document'..k,true,
	basel..'mute_inline'..k,true,
	basel..'lock_username'..k,true,
	basel..'replay'..k,true
	)
	redis:sadd(basel..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(basel..'username:'..ID,'username',user)
	redis:sadd(basel..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(basel..'username:'..ID,'username',user)
	redis:sadd(basel..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(basel..'username:'..ID,'username',user)
	redis:sadd(basel..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'•~** تم رفع آلنسـخه‏‏ آلآحتيآطـيه\n•~** حآليآ عدد مـجمـوعآتك هيه *'..redis:scard(basel..'group:ids')..'* ®️\n✓')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(basel..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(basel..'username:'..data.user_.id_).username
	...
	redis:hset(basel..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
	...
	local groups = redis:smembers(basel..'group:ids')
	...
	local pv = redis:smembers(basel..'users')
	local NumPvDel = 0
	for i = 1, #pv do
